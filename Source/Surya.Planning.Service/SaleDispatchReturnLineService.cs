...
namespace Surya.India.Service
{
    public interface ISaleDispatchReturnLineService : IDisposable
    {
        ...
        int NextId(int id);
        int PrevId(int id);
        IEnumerable<SaleDispatchReturnLineViewModel> GetPurchaseOrderForFilters(SaleDispatchReturnLineFilterViewModel vm);
        IEnumerable<SaleDispatchReturnLineViewModel> GetSaleDispatchsForFilters(SaleDispatchReturnLineFilterViewModel vm);
        IEnumerable<SaleDispatchListViewModel> GetPendingPurchaseReceiptHelpList(int Id, string term);//PurchaseOrderHeaderId
    ...
    public class SaleDispatchReturnLineService : ISaleDispatchReturnLineService
    {
        ...
        public IEnumerable<SaleDispatchReturnLineViewModel> GetPurchaseOrderForFilters(SaleDispatchReturnLineFilterViewModel vm)
        {
            ...
            var temp = (from p in db.ViewSaleDispatchBalance
                        ...
                        && (string.IsNullOrEmpty(vm.SaleOrderHeaderId) ? 1 == 1 : SaleOrderIdArr.Contains(p.PurchaseOrderHeaderId.ToString()))
        ...
        public IEnumerable<SaleDispatchListViewModel> GetPendingPurchaseReceiptHelpList(int Id, string term)
        {

            var PurchaseInvoice = new SaleDispatchReturnHeaderService(_unitOfWork).Find(Id);

            //var settings = new SaleDispatchSettingService(_unitOfWork).GetSaleDispatchSettingForDocument(PurchaseInvoice.DocTypeId, PurchaseInvoice.DivisionId, PurchaseInvoice.SiteId);
            ...
            var list = (from p in db.ViewSaleDispatchBalance
                        where (string.IsNullOrEmpty(term) ? 1 == 1 : p.SaleDispatchNo.ToLower().Contains(term.ToLower())) && p.BalanceQty > 0 && p.SupplierId == PurchaseInvoice.BuyerId
        ...
        public IEnumerable<PurchaseOrderLineListViewModel> GetPendingPurchaseOrderHelpList(int Id, string term)//PurchaseOrderHeaderId
        {
           ...
           // var settings = new SaleDispatchSettingService(_unitOfWork).GetSaleDispatchSettingForDocument(PurchaseInvoice.DocTypeId, PurchaseInvoice.DivisionId, PurchaseInvoice.SiteId);
            ...
            var list = (from p in db.ViewSaleDispatchBalance
                        where (string.IsNullOrEmpty(term) ? 1 == 1 : p.PurchaseOrderNo.ToLower().Contains(term.ToLower())) && p.BalanceQty > 0 && p.SupplierId == PurchaseInvoice.BuyerId && p.PurchaseOrderLineId !=null
                        //&& (string.IsNullOrEmpty(settings.filterContraDocTypes) ? 1 == 1 : contraDocTypes.Contains(p.DocTypeId.ToString()))
                        // && (string.IsNullOrEmpty(settings.filterContraSites) ? p.SiteId == CurrentSiteId : contraSites.Contains(p.SiteId.ToString()))
                   // && (string.IsNullOrEmpty(settings.filterContraDivisions) ? p.DivisionId == CurrentDivisionId : contraDivisions.Contains(p.DivisionId.ToString()))
