...
namespace Surya.India.Service
{
    ...
    public class ProductSupplierService : IProductSupplierService
    {
        ...
        public ProductSupplier GetProductSupplier(int pt)
        {
            ...
            return ProductSupplierRepository.Include(r => r.Product).Include(r => r.Supplier).Get().Where(i => i.ProductSupplierId == pt).FirstOrDefault();
        }
        ...
        {
            var pt = _unitOfWork.Repository<ProductSupplier>().Query().Include(m=>m.Supplier).Include(m=>m.Product).Get().Where(m => m.ProductId == id).ToList();
            return pt;
        }
