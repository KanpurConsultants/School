    ...
    public class Report_PackingPrintController : ReportController
    {
        ...
        public DataTable FGetDataForSCILabelPrint(DataTable DtTemp, SqlConnection con)
        {
           ...
           // Int16 K = 0;
            ...
            mQry = "CREATE TABLE [#" + bTempTable + "] " +
                    " (BarCode Image, CarpetSku nVarChar(100), SaleOrder nVarChar(100), SaleOrderHeaderId nVarChar(100), Serial nVarChar(100), UPCCode Image, ProductId nVarChar(100), PrintSign VarChar(2), BaleNo nVarChar(10), CarpetNo nVarChar(10), CarpetNoImg Image , BaleNoCarpetImg Image )";
            ...
            if (DtTemp.Rows.Count > 0)
            {
                for (I = 0; I <= DtTemp.Rows.Count - 1; I++)
                {
                    ...
                    for (J = 1; J <= Convert.ToDouble(DtTemp.Rows[I]["Qty"].ToString()); J++)
                    {
                        ...
                        String sSQL = "Insert Into [#" + bTempTable + "] (BarCode, SaleOrder, SaleOrderHeaderId, CarpetSku, Serial, UPCCode, ProductId, PrintSign, BaleNo, CarpetNo, CarpetNoImg, BaleNoCarpetImg) Values(@BarCode, @SaleOrder, @SaleOrderHeaderId, @CarpetSku, @Serial, @UPCCode, @ProductId, @PrintSign, @BaleNo, @CarpetNo, @CarpetNoImg, @BaleNoCarpetImg )";
                ...
                strQry = " Select H.BarCode, H.UPCCode, ISNULL(IB.BuyerSKU,H.CarpetSku) AS CarpetSku, ISNULL(H.SaleOrder,'') AS SaleOrder, H.SaleOrderHeaderId, ISNULL(H.Serial,'') AS Serial, H.ProductId, So.SaleToBuyerId, " +
                        ...
                        " H.BaleNo, H.CarpetNo, ISNULL(H.CarpetNoImg,'') AS CarpetNoImg , ISNULL(H.BaleNoCarpetImg,'') AS BaleNoCarpetImg " +
                        " From [#" + bTempTable + "] H " +
        ...
        public Image FRetUPCImage(String UPCCode)
        {
            ...
            if (UPCCode.Length == 12)
            {
                ...
                FRetUPCImage = upca.CreateBarCode(UPCCode, 2);
            }
            else
                FRetUPCImage = null;
